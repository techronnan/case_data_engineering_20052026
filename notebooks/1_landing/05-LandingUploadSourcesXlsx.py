...
# DBTITLE 1,LandingUploadSourcesXlsx
...
for fname in [f for f in EXPECTED_FILES if f.endswith('.xlsx')]:
    ok = fname in present
    if not ok:
...
print(f"[Landing XLSX] {_status} | {_duracao:.1f}s | {_total:,} registros")
if _todos_erros:
    raise Exception(f"Erros na landing XLSX: {_msg_erro}")
