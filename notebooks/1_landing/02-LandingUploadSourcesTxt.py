...
# DBTITLE 1,LandingUploadSourcesTxt
...
for fname in [f for f in EXPECTED_FILES if f.endswith('.txt')]:
    ok = fname in present
    if not ok:
...
print(f"[Landing TXT] {_status} | {_duracao:.1f}s | {_total:,} registros")
if _todos_erros:
    raise Exception(f"Erros na landing TXT: {_msg_erro}")
